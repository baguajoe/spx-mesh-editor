...
# ── 1. FootPlantSolver.js ─────────────────────────────────────────────────────
SOLVER = r'''// FootPlantSolver.js — Foot Plant Detection + IK Foot Lock
...
path = os.path.join(BASE, "FootPlantSolver.js")
with open(path, 'w') as f:
    f.write(SOLVER)
print(f"✅ {path}")

# ── 2. Patch MocapRetarget.js — replace stub fixFootSliding with real one ─────
RETARGET_PATH = os.path.join(BASE, "MocapRetarget.js")
with open(RETARGET_PATH, 'r') as f:
    content = f.read()

OLD_STUB = '''export function fixFootSliding(frames, threshold = 0.01) {
...
NEW_IMPL = '''export function fixFootSliding(frames, options = {}) {
...
if OLD_STUB in content:
    ...
    with open(RETARGET_PATH, 'w') as f:
        f.write(content)
    print(f"✅ MocapRetarget.js — stub replaced with FootPlantSolver delegate")
else:
    print(f"⚠️  MocapRetarget.js stub not found verbatim — appending instead")
    with open(RETARGET_PATH, 'a') as f:
        f.write('''
// ─── Real foot plant solver (replaces stub) ───────────────────────────────────
export { solveFootPlanting as fixFootSliding } from './FootPlantSolver.js';
export { detectFootPlants, getFootPlantStats, blendFootTransitions } from './FootPlantSolver.js';
''')
    print(f"✅ MocapRetarget.js — FootPlantSolver re-exports appended")

# ── 3. Patch MocapWorkspace — add foot plant UI + apply on export ─────────────
WORKSPACE_PATH = "/workspaces/spx-mesh-editor/src/workspaces/mocap/MocapWorkspace.jsx"
with open(WORKSPACE_PATH, 'r') as f:
    ws = f.read()

# Add import
OLD_IMPORT = "import { downloadBVH, MocapRetargeter } from '../../mesh/MocapRetarget.js';"
NEW_IMPORT = """import { downloadBVH, MocapRetargeter } from '../../mesh/MocapRetarget.js';
import { solveFootPlanting, getFootPlantStats } from '../../mesh/FootPlantSolver.js';"""

if OLD_IMPORT in ws and 'FootPlantSolver' not in ws:
    ws = ws.replace(OLD_IMPORT, NEW_IMPORT)
    print("✅ MocapWorkspace.jsx — FootPlantSolver import added")

# Add footPlant state after retargetStats state
OLD_STATE = "  const [retargetStats,   setRetargetStats]   = useState(null);"
NEW_STATE = """  const [retargetStats,   setRetargetStats]   = useState(null);
  const [footPlant,       setFootPlant]       = useState(true);
  const [footPlantStats,  setFootPlantStats]  = useState(null);"""

if OLD_STATE in ws and 'footPlant,' not in ws:
    ws = ws.replace(OLD_STATE, NEW_STATE)
    print("✅ MocapWorkspace.jsx — footPlant state added")

# Patch exportJSON to apply foot plant fix
OLD_EXPORT = """  const exportJSON = useCallback(() => {
...
NEW_EXPORT = """  const processFramesForExport = useCallback((frames) => {
...
if OLD_EXPORT in ws and 'processFramesForExport' not in ws:
    ws = ws.replace(OLD_EXPORT, NEW_EXPORT)
    print("✅ MocapWorkspace.jsx — exportJSON patched with foot plant fix")

# Patch BVH export button to also apply fix
OLD_BVH = "onClick={() => downloadBVH(recordedFrames, `mocap_${Date.now()}.bvh`)}"
NEW_BVH  = "onClick={() => downloadBVH(processFramesForExport(recordedFrames), `mocap_${Date.now()}.bvh`)}"
if OLD_BVH in ws:
    ws = ws.replace(OLD_BVH, NEW_BVH)
    print("✅ MocapWorkspace.jsx — BVH export patched with foot plant fix")

# Add foot plant UI after retargeting section
OLD_RETARGET_UI = "        {/* Avatar */}"
NEW_RETARGET_UI = """        {/* ── NEW: Foot Plant ── */}
...
if OLD_RETARGET_UI in ws and 'Foot Plant Fix' not in ws:
    ws = ws.replace(OLD_RETARGET_UI, NEW_RETARGET_UI)
    print("✅ MocapWorkspace.jsx — Foot Plant UI section added")

with open(WORKSPACE_PATH, 'w') as f:
    f.write(ws)
print(f"✅ MocapWorkspace.jsx written")

print("\n✅ All done — run: npm run build 2>&1 | grep 'MISSING_EXPORT|Error:' | head -20")
