...
panel_path = f"{base}/components/mesh/NodeCompositorPanel.jsx"
os.makedirs(os.path.dirname(panel_path), exist_ok=True)
with open(panel_path, "w") as f:
    f.write(panel)
print(f"✅ {panel_path}")

# ── 2. Wire NodeCompositorPanel into App.jsx ─────────────────────────────────
app_path = f"{base}/App.jsx"
with open(app_path, "r") as f:
    app = f.read()

# Import
if "NodeCompositorPanel" not in app:
    app = app.replace(
        "import SPX3DTo2DPanel",
        "import NodeCompositorPanel from './components/mesh/NodeCompositorPanel';\nimport SPX3DTo2DPanel"
    )
    print("✅ NodeCompositorPanel import added")

# Render — inject near SPX3DTo2DPanel
render_tag = """      <NodeCompositorPanel
        open={compositorOpen}
        onClose={() => setCompositorOpen(false)}
      />"""

if "NodeCompositorPanel" not in app or "compositorOpen}" not in app:
    app = app.replace(
        "      <SPX3DTo2DPanel",
        f"{render_tag}\n      <SPX3DTo2DPanel"
    )
    print("✅ NodeCompositorPanel render injected")

with open(app_path, "w") as f:
    f.write(app)

# ── 3. Draco GLB export in FBXPipeline.js ────────────────────────────────────
fbx_path = f"{base}/mesh/FBXPipeline.js"
with open(fbx_path, "r") as f:
    fbx = f.read()

if "DRACOExporter" not in fbx:
    # Add Draco export function at the end of FBXPipeline.js
    draco_fn = r"""
...
"""
    ...
    with open(fbx_path, "w") as f:
        f.write(fbx)
    print(f"✅ Draco GLB export added to FBXPipeline.js")
else:
    print("✅ DRACOExporter already present")

# ── 4. Wire Draco export into App.jsx handleApplyFunction ────────────────────
with open(app_path, "r") as f:
    app = f.read()

# Import exportGLBWithDraco
if "exportGLBWithDraco" not in app:
    # Find the FBXPipeline import line and add to it
    import re
    app = re.sub(
        r'(import\s*\{[^}]*)(exportGLB|exportOBJ)([^}]*\}\s*from\s*["\'].*FBXPipeline)',
        lambda m: m.group(0).replace("} from", ", exportGLBWithDraco } from", 1) if "exportGLBWithDraco" not in m.group(0) else m.group(0),
        app
    )
    print("✅ exportGLBWithDraco import added")

# Add handler
if "draco" not in app.lower():
    app = app.replace(
        'if (fn === "exportGLB")',
        'if (fn === "exportGLBDraco") { exportGLBWithDraco(sceneRef.current, { filename:"spx_draco.glb" }).then(()=>setStatus("✓ Draco GLB exported")); return; }\n    if (fn === "exportGLB")'
    )
    print("✅ Draco export handler added")

with open(app_path, "w") as f:
    f.write(app)

# ── 5. Add Draco export to File menu in ProfessionalShell ────────────────────
shell_path = f"{base}/pro-ui/ProfessionalShell.jsx"
with open(shell_path, "r") as f:
    shell = f.read()

if "exportGLBDraco" not in shell:
    shell = shell.replace(
    ...
    with open(shell_path, "w") as f:
        f.write(shell)
    print("✅ Draco export added to File menu")

print("\n── Done. NodeCompositorPanel + Draco GLB export complete ──")
